...
class Programator(Zamestnanec):
    ...
    def vypocitej_mzdu(self):
        bonus = 1000 * self.pocet_odpracovanych_let
        #return self.zakladni_mzda + bonus
        #rocenta = int(self.zakladni_mzda + bonus) * 0,1
       #procenta = int(procenta)
        procenta = (self.zakladni_mzda + bonus) * 0.1
        return self.zakladni_mzda + bonus + procenta
    ...
    def __str__(self):
        return f"Programator {self.jmeno}, odpracovanych let {self.pocet_odpracovanych_let}, mzda {self.zakladni_mzda} Kc"
...
class Manazer(Zamestnanec):
    def __init__(self, jmeno, zakladni_mzda, pocet_nadrizenych):
        super().__init__(jmeno, zakladni_mzda)
        self.pocet_nadrizenych = pocet_nadrizenych
        pocet_nadrizenych = int(pocet_nadrizenych)
        pocet_nadrizenych = 0
    ...
    def vypocitej_mzdu(self):
        # Zakladni mzda + 1000 Kc za kazdy odpracovany rok
        bonus = 1000 * self.pocet_odpracovanych_let
        return self.zakladni_mzda + bonus + (self.pocet_nadrizenych * 1000)

    def __str__(self):
        return f"Manazer {self.jmeno}, odpracovanych let {self.pocet_odpracovanych_let}, mzda {self.zakladni_mzda} Kc"
